...
def classify_question(state):
    """
    Uses the LLM to classify which type of question to generate next.
    This node also acts as a data labeler for our future PyTorch model.
    This version uses a simpler prompt and expects a single character response for robustness.
    """
    print("---NODE: CLASSIFYING QUESTION TYPE (LLM-BASED)---")
    llm = state['llm']
    document = state.get("document", "")
    
    # Use a smaller chunk of the document for faster classification
    document_chunk = document[:4000]

    # A more balanced and nuanced prompt to guide the LLM's choice.
    classifier_prompt = PromptTemplate.from_template(
        """You are a data scientist creating a test set for a RAG system. Analyze the following document and choose the most appropriate question type to generate.
        Your answer must be a single character: 'A', 'B', or 'C'.

        **Analysis Criteria:**
        1.  **Content Structure:** Does the text contain distinct, separable sections that could be compared? (Leads to C)
        2.  **Information Density:** Is the text a high-level overview, likely missing specific details a user might ask? (Leads to B)
        3.  **Specificity:** Does the text contain many specific, self-contained facts, numbers, or definitions? (Leads to A)

        **Decision Logic:**
        - **Choose 'C' (Synthesis/Comparison):** ONLY if the document is long (>3000 chars) AND clearly contains multiple, distinct sub-topics that can be compared or synthesized. Example: A biography covering a person's early life and their later career.
        - **Choose 'B' (No-Context):** If the document gives a general overview of a broad topic, making it likely that a user would ask for details not present. Example: An article about "The History of Artificial Intelligence" that doesn't mention specific algorithms.
        - **Choose 'A' (Factual):** If the document is short, or if it focuses on a very specific topic with clear, extractable facts. This should be the default choice if you are uncertain.
        ...
        Based on these criteria, what is the single best question type? Respond with only A, B, or C.
        """
    )

    # Create the classification chain. We don't need structured output here.
    classification_chain = classifier_prompt | llm
    
    try:
        # Invoke the chain and get the raw string content
        result = classification_chain.invoke({"document": document_chunk})
        # .content is used to get the string from an AIMessage
        raw_response = result.content.strip().upper()
        
        # Extract the first valid character from the response
        question_type = ""
        ...
        if not question_type:
            print(f"  - Warning: LLM response '{raw_response}' contained no valid type. Defaulting to 'A'.")
            question_type = "A"

    except Exception as e:
        print(f"  - Error during LLM classification: {e}. Defaulting to 'A'.")
        question_type = "A"

    # Additional logic to prevent Type C for short documents, as a safeguard
    if question_type == "C" and len(document) < 4000:
        print(f"  - Warning: LLM chose Type C for a short document ({len(document)} chars). Overriding to 'A'.")
        question_type = "A"

    print(f"  - LLM Decision: Generate a Type {question_type} question.")
